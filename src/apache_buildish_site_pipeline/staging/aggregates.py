...
def finalize_pages_and_write_aggregates(
    *,
    ...
    page_contributions = tuple(
        page for manifest in unit_contribution_manifests for page in manifest.pages
    )
    component_front_matter_by_slug = {
        component.slug: build_component_front_matter(
...
def _finalize_staged_pages(
    *,
    ...
    for contribution in page_contributions:
        staged_page_path = _normalized_stage_relative_path(
            layout, contribution.stage_relative_path
        )
        finalize_staged_page(
...
def _translations_by_page(
    ...
    grouped: dict[tuple[str, str | None, str], list[StagedPageContributionWire]] = (
        defaultdict(list)
    )
    for contribution in page_contributions:
        if (
            contribution.translation_key is None
            or contribution.locale is None
            or contribution.public_url is None
        ):
            continue
        grouped[
            (
                contribution.component_slug,
                contribution.artifact_key,
                contribution.translation_key,
            )
        ].append(contribution)
    ...
    for group in grouped.values():
        links = [
            build_translation_link(contribution)
            for contribution in sorted(group, key=lambda item: item.locale or "")
        ]
        for contribution in group:
            links_by_page[_page_identity(contribution)] = [
                link for link in links if link.locale != contribution.locale
            ]
...
def _write_aggregate_files(
    *,
    ...
    artifacts_path = _write_items_file(
        data_root / "artifacts.json", _build_artifacts_entries(build_plan)
    )
    routes_path = _write_items_file(
        data_root / "routes.json", _build_route_entries(build_plan)
    )
    redirects_path = _write_items_file(
        data_root / "redirects.json", _build_redirect_entries(build_plan)
    )
    providers_path = (
        _write_items_file(
            data_root / "providers.json", _build_provider_entries(provider_snapshot)
        )
    ...
    releases_path = (
        _write_items_file(data_root / "releases.json", releases) if releases else None
    )
    candidates_path = (
        _write_items_file(data_root / "candidates.json", candidates)
        if candidates
        else None
    )
    refs_path = _write_items_file(data_root / "refs.json", refs) if refs else None
    translations_path = (
        _write_items_file(data_root / "translations.json", translations)
        if translations
        else None
    )
    compatibility_path = (
        _write_items_file(data_root / "compatibility.json", compatibility)
        if compatibility
        else None
    )
    mounts_path = (
        _write_items_file(data_root / "mounts.json", mounts) if mounts else None
    )
    content_index_path = (
        _write_items_file(data_root / "content-index.json", content_index)
        if content_index
        else None
    )
    ...
    data_files = StageDataFiles(
        ...
        diagnostics=str(diagnostics_path.relative_to(layout.next_stage_root))
        if diagnostics_path is not None
        else None,
        unit_contributions=str(
            unit_contributions_path.relative_to(layout.next_stage_root)
        ),
        output_ownership=str(output_ownership_path.relative_to(layout.next_stage_root)),
        aggregate_dependencies=str(
            aggregate_dependencies_path.relative_to(layout.next_stage_root)
        ),
    )
    _write_json_file(
        unit_contributions_path,
        PersistedUnitContributionsV1(units=unit_contribution_manifests),
    )
    _write_json_file(
        output_ownership_path,
        build_output_ownership_map(units=owned_units, data_files=data_files),
    )
    _write_json_file(
        aggregate_dependencies_path,
        build_aggregate_dependency_map(units=owned_units, data_files=data_files),
    )
    return data_files
...
def _build_components_entries(
    ...
    for context in build_plan.selected_versions:
        if context.provider_record is not None:
            provider_keys_by_component[context.component_slug].add(
                context.provider_record.provider
            )
    entries = []
    for component in build_plan.site.components:
...
def _build_artifacts_entries(
    build_plan: EffectiveBuildPlan,
) -> list[ArtifactsDataEntry]:
    selected_by_artifact: dict[tuple[str, str], list[SelectedVersionContext]] = (
        defaultdict(list)
    )
    for context in build_plan.selected_versions:
    ...
    for component in build_plan.site.components:
        ...
        artifact_front_matter_by_key = {
            ...
            for item in (
                build_component_front_matter(
                    component, tuple(component_contexts)
                ).artifacts
                or ()
            )
        }
        for artifact in component.artifacts:
            ...
            released_contexts = [
                context for context in contexts if context.kind is RecordKind.RELEASED
            ]
            candidate_contexts = [
                context for context in contexts if context.kind is RecordKind.CANDIDATE
            ]
            named_refs = [
                context
                for context in contexts
                if context.kind
                in {RecordKind.NAMED_REF, RecordKind.LINE_HEAD, RecordKind.DEVELOPMENT}
            ]
            ...
            entries.append(
                ArtifactsDataEntry(
                    ...
                    latest_stable=artifact.lifecycle.latest_stable
                    if artifact.lifecycle is not None
                    else None,
                    latest_release=latest_release,
                    latest_candidate=latest_candidate,
                    release_lines=artifact_front_matter.release_lines
                    if artifact_front_matter is not None
                    else None,
                    named_refs=[
...
def _build_release_entries(
    ...
    for context in build_plan.selected_versions:
        ...
        artifact = _artifact_for_context(
            component_by_slug[context.component_slug], context
        )
        entries.append(
            ReleaseAggregateEntry(
                ...
                release_line_ancestors=list(
                    provider_record.release_line_ancestors or ()
                ),
                withdrawal_behavior=context.withdrawal_behavior,
                redirect_target=context.redirect_target,
                maturity=provider_record.maturity,
                assets=list(provider_record.assets)
                if provider_record.assets is not None
                else None,
...
def _build_translation_entries(
    page_contributions: tuple[StagedPageContributionWire, ...],
) -> list[TranslationSetAggregateEntry]:
    groups: dict[tuple[str, str | None, str], list[StagedPageContributionWire]] = (
        defaultdict(list)
    )
    for contribution in page_contributions:
        if (
    ...
    entries = []
    for (component_slug, artifact_key, translation_key), group in sorted(
        groups.items()
    ):
    ...
    for component in build_plan.site.components:
        ...
        for artifact in component.artifacts:
            for assertion in _artifact_compatibility_entries(artifact):
                entries.append(
                    _compatibility_entry(component.slug, artifact.key, assertion)
                )
    return entries
...
def _build_content_index_entries(
    ...
    for contribution in page_contributions:
        ...
        if page_url is None:
            raise StageIntegrityError(
                f"Content index entry is missing a public URL: {contribution.stage_relative_path}"
            )
        ...
        entries.append(
            ContentIndexEntry(
                ...
                external_id=(
                    provider_context.get("externalId")
                    if provider_context is not None
                    else None
                ),
                ...
                version_kind=RecordKind(contribution.version_kind)
                if contribution.version_kind is not None
                else None,
...
def _compatibility_entry(
    ...
    assertion: CompatibilityAssertionConfig,
) -> CompatibilityAggregateEntry:
    subject_id = (
        assertion.subject_ref
        if artifact_key is None
        else f"artifact:{component_slug}:{artifact_key}:{assertion.subject_ref}"
    )
    target_id = (
        assertion.target_ref
        if artifact_key is None
        else f"artifact:{component_slug}:{artifact_key}:{assertion.target_ref}"
    )
...
def _mount_entry(
    component_slug: str, artifact_key: str | None, mount: MountConfig
) -> MountAggregateEntry:
    owner_id = mount.ownership or (
        f"artifact:{component_slug}:{artifact_key}"
        if artifact_key is not None
        else f"component:{component_slug}"
    )
...
def _latest_release_summary(
    contexts: list[SelectedVersionContext],
) -> LatestReleaseSummary | None:
    if not contexts:
        return None
    context = max(
        contexts,
        key=lambda item: (
            (_provider_record_for_context(item).published_at or ""),
            item.version or "",
        ),
    )
    provider_record = _provider_record_for_context(context)
...
def _latest_candidate_summary(
    contexts: list[SelectedVersionContext],
) -> LatestCandidateSummary | None:
    if not contexts:
        return None
    context = max(
        contexts,
        key=lambda item: (
            _provider_record_for_context(item).candidate_sequence or 0,
            item.version or "",
        ),
    )
    provider_record = _provider_record_for_context(context)
...
def _artifact_for_context(
    ...
    for artifact in component.artifacts:
        if artifact.key == context.artifact_key:
            return artifact
    raise ValueError(
        f"Artifact {context.artifact_key} not found for component {component.slug}"
    )


def _support_status_for_context(
    ...
    for release_line in artifact.lifecycle.release_lines or ():
        if (
            ...
            and release_line.key == provider_record.release_line
            and release_line.support_status is not None
        ):
...
def _support_window_for_context(
    ...
    for release_line in artifact.lifecycle.release_lines or ():
        if (
            provider_record is not None
            and release_line.key == provider_record.release_line
            and release_line.support_window is not None
        ):
...
def _artifact_docs_root(artifact: ResolvedArtifactConfig, workspace_root: Path) -> str:
    ...
    raise StageIntegrityError(
        f"Artifact docs root must remain repo-relative: {artifact.key}"
    )
...
def _provider_record_for_context(
    ...
    if context.provider_record is None:
        raise StageIntegrityError(
            f"Selected version context is missing provider metadata: {context.component_slug}:{context.artifact_key}"
        )
...
def _required_context_ref(context: SelectedVersionContext) -> str:
    if context.ref is None:
        raise StageIntegrityError(
            f"Selected version context is missing ref: {context.component_slug}:{context.artifact_key}"
        )
    return context.ref
...
def _validated_unit_manifest_path(
    layout: WorkRootLayout, result: WorkerResultWire
) -> Path | None:
    ...
    if raw_manifest_path.is_symlink():
        raise StageIntegrityError(
            f"Worker contribution manifest must be one normal file: {raw_manifest_path}"
        )

    fragments_root = layout.fragments_root.resolve(strict=False)
    expected_path = (
        layout.fragments_root / f"{result.unit_id.replace(':', '_')}.json"
    ...
    if not manifest_path.is_relative_to(fragments_root):
        raise StageIntegrityError(
            f"Worker contribution manifest escapes the private fragment root: {manifest_path}"
        )
    if manifest_path.is_symlink() or not manifest_path.is_file():
        raise StageIntegrityError(
            f"Worker contribution manifest must be one normal file: {manifest_path}"
        )
...
def _page_identity(
    ...
    return (
        contribution.component_slug,
        contribution.artifact_key,
        contribution.stage_relative_path,
    )


def _normalized_stage_relative_path(
    layout: WorkRootLayout, stage_relative_path: str
) -> Path:
...
def _write_json_file(path: Path, value: Any) -> None:
    ...
    if hasattr(value, "model_dump_json"):
        ...
        serialized = '{\n  "items": [\n'
        serialized += ",\n".join(
            _serialize_item(item, indent="    ") for item in value["items"]
        )
        serialized += "\n  ]\n}"
    else:
        serialized = (
