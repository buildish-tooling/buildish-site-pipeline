...
Version: current unreleased `buildish-site-pipeline` development branch.
*(documented)*

Date: 2026-08-04. *(documented)*

Authors: Buildish maintainers. *(documented)*

Reporting cross-reference: findings that may violate §8 should be reported per
this repository's `SECURITY.md` disclosure channel. Sections §3 and §9 inform
triage, but maintainers make the final scope determination. *(documented)*

Status: development draft and review aid. No Buildish release has been published;
this model must be reviewed again before it is attached to a release.
*(documented)*
...
| `site/catalog.yaml` | Catalog document fields | Yes, if repository content is attacker-influenced | Review source roots, publication paths, and redirects before CI use. *(documented)* |
...
- Adds a preview server, renderer integration, template execution, subprocess
  ...
  *(inferred)*
- Changes defaults or operator policy for §5a resource limits, report/event
  sinks, or stage/work roots. *(inferred)*
...
## §14 Optional machine-readable companion
