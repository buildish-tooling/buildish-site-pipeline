...
def select_version_contexts(
    *, site: ResolvedSiteConfig, provider_index: ProviderSnapshotIndex
) -> SelectedVersionSet:
    ...
    for component in site.components:
        for artifact in component.artifacts:
            effective_policy = (
                ...
                or component.publication_selection
                or _default_publication_selection_policy()
            )
            provider_context = provider_index.contexts_by_artifact.get(
                (component.slug, artifact.key)
            )
            selected_contexts.extend(
                _select_development_context(
                    component.slug, artifact, provider_context, effective_policy
                )
            )
            line_head_contexts = _select_line_head_contexts(
                component.slug, artifact, provider_context, effective_policy
            )
            ...
            candidate_contexts = _select_candidate_contexts(
                component.slug, artifact, provider_context, effective_policy
            )
            selected_contexts.extend(candidate_contexts)
            deterministic = deterministic and all(
                context.deterministic
                for context in line_head_contexts + candidate_contexts
            )
...
def _select_line_head_contexts(
    ...
    authored_release_lines = (
        {line.key: line for line in artifact.lifecycle.release_lines or ()}
    ...
    for line_key in selected_line_keys:
        ...
        if provider_context is not None:
            provider_records = list(
                provider_context.line_heads_by_release_line.get(line_key, ())
            )
            if provider_records:
        ...
        selected_contexts.append(
            SelectedVersionContext(
                ...
                ref=authored_line.maintenance_ref
                or (provider_record.ref if provider_record else None),
                commit_sha=provider_record.commit_sha if provider_record else None,
                support_status=authored_line.support_status
                or (provider_record.support_status if provider_record else None),
                provider_record=provider_record,
                deterministic=deterministic,
            ),
        )
...
def _select_release_contexts(
    ...
    authored_releases = (
        {release.version: release for release in artifact.lifecycle.releases or ()}
        if artifact.lifecycle
        else {}
    )
    ...
    if release_policy.mode is ReleaseSelectionMode.EXPLICIT:
        ...
        selected_versions = (
            tuple(
                line.latest
                for line in artifact.lifecycle.release_lines or ()
                if line.latest is not None
            )
            if artifact.lifecycle
            else ()
        )
    ...
    for version in selected_versions:
        provider_records = (
            list(provider_context.released_by_version.get(version, ()))
            if provider_context is not None
            else []
        )
        provider_record = (
            _choose_best_record(provider_records) if provider_records else None
        )
        ...
        selected_contexts.append(
            SelectedVersionContext(
                ...
                if provider_record
                else (authored_release.publication_state if authored_release else None),
                withdrawal_behavior=authored_release.withdrawal_behavior
                if authored_release
                else None,
                redirect_target=authored_release.redirect_target
                if authored_release
                else None,
                provider_record=provider_record,
                deterministic=len(provider_records) <= 1,
...
def _select_named_ref_contexts(
    ...
    authored_named_refs = {
        named_ref.key: named_ref for named_ref in artifact.versioning.named_refs or ()
    }
    selected_contexts = []
    for named_ref_key in selected_keys:
        ...
        if provider_context is not None:
            provider_records = list(
                provider_context.named_refs_by_key.get(named_ref_key, ())
            ) or list(provider_context.refs_by_ref.get(authored_named_ref.ref, ()))
            if provider_records:
                provider_record = _choose_best_record(provider_records)
                deterministic = len(provider_records) == 1
        selected_contexts.append(
            SelectedVersionContext(
                ...
                display_version=provider_record.display_version
                if provider_record
                else authored_named_ref.display_name,
...
def _select_candidate_contexts(
    ...
    if (
        ...
        or candidate_policy.mode is CandidateSelectionMode.NONE
    ...
    for version in selected_versions:
        ...
        if candidate_policy.external_ids:
            provider_records = [
                ...
                if record.external_id in set(candidate_policy.external_ids)
            ]
...
def _choose_best_record(
    records: Sequence[IndexedProviderRecord],
) -> IndexedProviderRecord:
    return sorted(records, key=_provider_record_sort_key, reverse=True)[0]


def _provider_record_sort_key(
    record: IndexedProviderRecord,
) -> tuple[tuple[object, ...], ...]:
    return (
