...
from apache_buildish_site_pipeline.models.catalog import ArtifactLifecycleConfig, ExactReleaseConfig, ReleaseLineConfig
...
def build_component_front_matter(
    ...
    for artifact in component.artifacts:
        ...
        release_lines = _release_line_summaries(artifact, selected_by_artifact.get(artifact.key, []))
        artifact_summaries.append(
            ArtifactFrontMatterSummary(
                key=artifact.key,
                display_name=_artifact_display_name(artifact),
                latest_stable=lifecycle.latest_stable if lifecycle is not None else None,
...
def build_version_context(component: ResolvedComponentConfig, context: SelectedVersionContext) -> VersionContext:
    ...
    release_line_key = context.release_line or (provider_record.release_line if provider_record is not None else None)
    ...
    if release_line_key is not None:
        release_line = ReleaseLineContext(
            ...
            support_status=(exact_release.support_status if exact_release is not None else None)
            or (line_config.support_status if line_config is not None else None),
            ancestors=list(provider_record.release_line_ancestors) if provider_record is not None else None,
            support_window=line_config.support_window if line_config is not None else None,
        )
    ...
    return VersionContext(
        ...
        url=_public_url_for_path(publication.origin.base_url, public_path_for_context(publication=publication, context=context)),
        docs_path=public_path_for_context(publication=publication, context=context),
        docs_url=_public_url_for_path(publication.origin.base_url, public_path_for_context(publication=publication, context=context)),
        tag=context.tag or (provider_record.tag if provider_record is not None else None),
        ref=context.ref,
        named_ref_key=context.named_ref_key if context.kind.value == "namedRef" else None,
        publication_state=context.publication_state or (provider_record.publication_state if provider_record is not None else None),
        maturity=context.maturity or (provider_record.maturity if provider_record is not None else None),
        candidate_sequence=provider_record.candidate_sequence if provider_record is not None else None,
        vote_status=provider_record.vote_status if provider_record is not None else None,
...
def finalize_staged_page(
    *,
    ...
    post = _load_authored_post(source_path=staged_page_path, allow_existing_pipeline=True)
    metadata = dict(post.metadata)
...
def extract_page_translation_key(metadata: Mapping[str, object]) -> str | None:
    ...
    candidate = metadata.get("translationKey") if "translationKey" in metadata else metadata.get("translation_key")
...
def detect_locale(relative_source_path: Path, localization: ResolvedLocalizationPolicy | None) -> tuple[str | None, bool, Path]:
    ...
    if parts and parts[0] in supported_locales:
        locale = parts[0]
        return locale, locale == localization.default_locale, Path(*parts[1:]) if len(parts) > 1 else Path()
    return localization.default_locale, True, relative_source_path
...
def build_page_front_matter(
    *,
    ...
    alternate_urls = None
    if translations:
        alternate_urls = sorted({translation.url for translation in translations if translation.url != contribution.public_url}) or None
    ...
    if page_url is None:
        raise StageIntegrityError(f"Staged page contribution is missing a public URL: {contribution.stage_relative_path}")
    component_url = contribution.component_url or contribution.public_url or contribution.canonical_url
    ...
    return PipelinePageFrontMatter(
        ...
        version=VersionContext.model_validate(normalized_version_context) if normalized_version_context is not None else None,
        provider=_provider_provenance(contribution),
    )


def build_translation_link(contribution: StagedPageContributionWire) -> TranslationLinkSummary:
...
def public_page_url(base_url: str | None, public_path: str, *, route_base_path: str | None = None) -> str | None:
...
def _load_authored_post(*, source_path: Path, allow_existing_pipeline: bool) -> frontmatter.Post:
...
def _artifact_lifecycle(component: ResolvedComponentConfig, *, artifact_key: str) -> ArtifactLifecycleConfig | None:
    for artifact in component.artifacts:
...
def _release_line_config(lifecycle: ArtifactLifecycleConfig | None, *, release_line_key: str | None) -> ReleaseLineConfig | None:
...
def _exact_release_config(lifecycle: ArtifactLifecycleConfig | None, *, version: str | None) -> ExactReleaseConfig | None:
...
def _provider_provenance(contribution: StagedPageContributionWire) -> ProviderProvenance | None:
    ...
    return ProviderProvenance(
        key=key,
        external_id=provider.get("externalId") if isinstance(provider.get("externalId"), str) else provider.get("external_id") if isinstance(provider.get("external_id"), str) else None,
        external_url=provider.get("externalUrl") if isinstance(provider.get("externalUrl"), str) else provider.get("external_url") if isinstance(provider.get("external_url"), str) else None,
    )
